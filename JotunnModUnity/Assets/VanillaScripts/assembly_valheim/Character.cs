...
public class Character : MonoBehaviour, IDestructible, Hoverable, IWaterInteractable
{
		...
		if (m_nview.IsOwner())
		{
			if (m_groundTilt != 0)
			{
				if (!IsFlying() && IsOnGround())
				{
					...
					m_visual.transform.localRotation = Quaternion.RotateTowards(m_visual.transform.localRotation, to, dt * 100f);
				}
				else
				{
					...
					if (IsSwiming())
					{
						m_visual.transform.localRotation = Quaternion.RotateTowards(m_visual.transform.localRotation, Quaternion.identity, dt * 50f);
					}
					else
					{
						m_visual.transform.localRotation = Quaternion.RotateTowards(m_visual.transform.localRotation, Quaternion.identity, dt * 100f);
					}
				}
	...
	protected bool AddStaggerDamage(float damage, Vector3 forceDirection)
	{
		...
		ZLog.Log("Adding stagger damage:" + damage);
